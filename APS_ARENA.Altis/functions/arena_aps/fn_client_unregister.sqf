...
{
 if ( saro_arena_debug ) then { diag_log "SASPS CUR-Server: No array. Player is not registered."; };
};

//something went wrong, leave
if (saro_clients_running_arena isEqualTo [] ) exitWith 
{
 if ( saro_arena_debug ) then { diag_log "SASPS CUR-Server: Array empty. Player is not registered."; };
};
...
{
  if ( saro_arena_debug ) then { diag_log "SASPS CUR-Server: Players last vehicle. Player entry deleted."; };
 ...
 {
  if ( saro_arena_debug ) then { diag_log "SASPS CUR-Server: Last registered player. Array niled, EH deleted."; };
...
{
if ( saro_arena_debug ) then { diag_log "SASPS CUR-Server: Player found but vehicle is not registered."; };
};

if ( saro_arena_debug ) then { diag_log "SASPS CUR-Server: Vehicle found and deleted in player entry."; };
